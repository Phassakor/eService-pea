...
import { dataTime, dataPackage } from "@/public/Data/dataForm";
import { voltageData } from "@/public/Data/dataForm";
...
export default function ServiceFormS307({ serviceId }: ServiceFormProps) {
    ...
    (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
      ...
      if (name === "address_province" || name === "address_city") {
        ...
        const inputValue: string | boolean =
          type === "checkbox" && name !== "transformer_voltage"
            ? checked
            : name === "transformer_voltage" && !checked
            ? ""
            : value;
        setFormData((prev) => ({
          ...prev,
      ...
      <form ref={formRef}>
        <CardHeader className="padding-card-header">
          <div className="text-header-card">
            สนใจรับบริการ ขอแก้ไข/บำรุงรักษาระบบจำหน่าย โดย HotLine
          </div>
        </CardHeader>
        <PersonalInfoForm
          formData={formData}
        ...
        <CardBody className="padding-card-body">
          <div className="gap-[1.75rem_1.875rem]">
            ...
            <div className="text-sub-title-card my-5">เลือก Package</div>
            <div className="text-label">* เลือกได้ 1 รายการ</div>
            <div className="box-card-time">
              {voltageData.map((item, index) => (
                <CheckBox
                  ...
                  value={item.value}
                  isChecked={formData.transformer_voltage === item.value}
                  isInvalid={
                    isInvalid && formData.transformer_voltage
                      ? !isInvalid
                      : isInvalid
                  }
                />
          ...
          <ButtonGroup
            pathRedirect="service/197b92d5-84f4-4786-3ea7-08dc84fff09f"
